...
- [ ] **`wedding_id` scoping sweep** in `lib/data/seating.ts` and `lib/data/comms.ts` — audit Tier 1
      item 7. Done when: `grep -c 'eq("wedding_id"' lib/data/seating.ts` is ≥ 8 and a cross-wedding
      `tableId` throws instead of writing.
- [ ] **Seven unguarded seating actions** — audit Tier 1 item 8. Done when:
      `grep -c unstable_rethrow "app/admin/(dashboard)/seating/actions.ts"` is 7.
- [ ] **Cache invalidation moved into the data layer** — audit Tier 1 item 5. Done when:
      `grep -rn "invalidateCache" lib/data/ | wc -l` is ≥ 5 and adding a guest updates Overview
      immediately rather than after 60 seconds.
- [ ] **The two deadline/timezone date bugs** — audit Tier 1 item 6. Done when: a test pins the
      "1 day" and "closed" boundaries at venue-local time, and the guest RSVP page renders the same
      date string before and after hydration with no console warning.
